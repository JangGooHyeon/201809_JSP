...
public class LoginCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		//전처리
		//1. session에 userVo(LoginServlet에서 정상 로그인 처리 되었을 시 저장한 속성)가 있는지 확인(정상적으로 로그인 한 상태인지 확인)
		...
		//2-2. 로그인 상태가 아니라면 --> 로그인 화면으로 이동(/login get방식)
		//*** 모든 페이지에 대해 적용 해야하나?
		//session이 필요한 페이지와 필요 없는 페이지 구분 필요
		// session 검증이 필요없는 요청(대다수는 검증이 필요함)
		// ** login 페이지 화면요청 session검증이 필요 없음
		// ** login 요청 처리(/login post) session 검증이 필요 없음
		// ** /js/*, /css/* 하위 모든 파일 
		// ex) 사용자가 localhost/login을 요청
		else {
